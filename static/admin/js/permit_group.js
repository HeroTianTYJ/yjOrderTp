...
function listItem (item) {
  ...
  return '<tr class="item' + item['id'] + '"><td' + (isPermission('delete') ? '' : ' class="none"') + '><div class="check-box"><label><input type="checkbox" name="id" value="' + item['id'] + '"></label></div></td><td>' + item['name'] + '</td><td style="text-align:left;">' + item['permit_manage'] + '</td><td style="text-align:left;">' + item['permit_data'] + '</td>' + (isPermission('isDefault') ? '<td class="is_default">' + (item['is_default'] ? '<span class="red">是</span>' : '<a href="javascript:" class="is_default">设为默认</a>') + '</td>' : '') + '<td>' + item['date'] + '</td>' + (control.length ? '<td>' + control.join('/') + '</td>' : '') + '</tr>';
}
