$(function () {
  ...
  $list.on('click', 'a.recover', function () {
    commonAjax(ThinkPHP['RECOVER'], {id: $(this).parent().parent().find('input[name=id]').val()});
    $(this).parent().parent().remove();
  });

  // 批量还原
  $tool.find('.recover').on('click', function () {
    commonAjax(ThinkPHP['RECOVER'], {ids: $tool.find('input[name=ids]').val()});
  });
});
...
function listItem (item) {
  ...
  if (ThinkPHP['CONTROLLER'] === 'OrderRecycle' && isPermission('recover')) control.push('<a href="javascript:" class="recover">还原</a>');
  if (isPermission('delete')) control.push('<a href="javascript:" class="delete">删除</a>');
  return '<tr class="item' + item['id'] + '"><td' + (isPermission('delete') ? '' : ' class="none"') + '><div class="check-box"><label><input type="checkbox" name="id" value="' + item['id'] + '"></label></div></td><td>' + item['order_id'] + '</td><td>' + item['manager'] + '</td><td>' + item['name'] + '</td><td>' + item['product'] + '</td><td>' + item['price'] + '元</td><td>' + item['count'] + '</td><td>' + item['total'] + '元</td><td>' + item['tel'] + '</td><td title="' + item['address'] + '">' + item['address_truncate'] + '</td><td>' + item['ip'] + '</td><td>' + item['date'] + '</td><td>' + item['payment'] + '</td><td>' + item['pay_id'] + '</td><td>' + item['pay_scene'] + '</td><td>' + item['pay_date'] + '</td><td>' + item['order_state'] + '</td><td>' + item['express'] + '</td>' + (control.length ? '<td>' + control.join('/') + '</td>' : '') + '</tr>';
}
