...
class Product extends Base
{
    public function index()
    {
        ...
        if (Request::isAjax()) {
            ...
            return $productAll->items() ? apiResponse('', 1, $productAll->items()) : '';
        }
        ...
        if (Request::isAjax()) {
            if (Request::get('action') == 'do') {
                ...
                if (is_numeric($productAdd)) {
                    return $productAdd > 0 ? apiResponse('商品添加成功！') : apiResponse('商品添加失败！', 0);
                } else {
                    return apiResponse($productAdd, 0);
                }
            }
            Html::productSort();
            return $this->view();
    ...
    public function update()
    {
        if (Request::isAjax() && Request::post('id')) {
            ...
            if (!$productOne) {
                return apiResponse('不存在此商品！', 0);
            }
            if (Request::get('action') == 'do') {
                if (Config::get('app.demo') && Request::post('id') <= 5) {
                    return apiResponse('演示站，id<=5的商品无法修改！', 0);
                }
                $productModify = $Product->modify();
                return is_numeric($productModify) ?
                    apiResponse('商品修改成功！', 1, $this->listItem($Product->one())) :
                    apiResponse($productModify, 0);
            }
    ...
    public function delete()
    {
        if (Request::isAjax() && (Request::post('id') || Request::post('ids'))) {
            if (Config::get('app.demo')) {
                return apiResponse('演示站，商品无法删除！', 0);
            }
            $Product = new model\Product();
            if (Request::post('id')) {
                if (!$Product->one()) {
                ...
                foreach (explode(',', Request::post('ids')) as $value) {
                    if (!$Product->one($value)) {
                        return apiResponse('不存在您勾选的商品！', 0);
                    }
                }
            }
            return $Product->remove() ? apiResponse('商品删除成功！') : apiResponse('商品删除失败！', 0);
        } else {
            return apiResponse('非法操作！', 0);
        }
    }
    ...
    public function isView()
    {
        if (Request::isAjax() && Request::post('id')) {
            if (Config::get('app.demo')) {
                return apiResponse('演示站，商品无法设置上下架！', 0);
            }
            $Product = new model\Product();
            $productOne = $Product->one();
            if (!$productOne) {
            ...
            if ($productOne['is_view'] == 0) {
                return $Product->isView(1) ? apiResponse('商品上架成功！') : apiResponse('商品上架失败！', 0);
            } else {
                return $Product->isView(0) ? apiResponse('商品下架成功！') : apiResponse('商品下架失败！', 0);
            }
        } else {
            return apiResponse('非法操作！', 0);
        }
    ...
    public function isDefault()
    {
        if (Request::isAjax() && Request::post('id')) {
            ...
            return $Product->isDefault() ? apiResponse('设置默认商品成功！') : apiResponse('设置默认商品失败！', 0);
        } else {
            return apiResponse('非法操作！', 0);
        }
    }
