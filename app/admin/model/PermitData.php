...
class PermitData extends Model
{
    //查询所有（主权限）
    public function all()
    {
        try {
            $all = $this->field('id,name,is_default')->where(['parent_id' => 0]);
    ...
    {
        try {
            $all = $this->field('id,name,alias')->order(['id' => 'ASC'])->where(['parent_id' => 0]);
    ...
    {
        try {
            return $this->field('id,name,alias,is_default')
                ->where('name', 'LIKE', '%' . Request::get('keyword') . '%')
