...
class Template
{
    public function html($id, $demo = 0)
    {
        ...
        $dd = '<input type="hidden" name="template_id" value="' . $id . '"><input type="hidden" name="price">' .
            '<input type="hidden" name="referrer">';
        $fieldTemp = explode(',', $templateOne['field_ids']);
        if (in_array($templateOne['template'], [0, 1, 2, 3])) {
            $dd .= '<dd class="center">请认真填写订单信息，以便您尽快收到货物</dd>';
            $dd .= '<dd class="pro"><span class="left">订购商品：</span><span class="right">' .
                $productHtml . '</span></dd>';
            $dd .= in_array(1, $fieldTemp) ? '<dd><span class="left">订购数量：</span><span class="right">' .
                '<input type="text" name="count" value="1" class="text"></span></dd>' :
                '<input type="hidden" name="count" value="1">';
            $dd .= '<dd><span class="left">总价合计：</span><span class="right">' .
                '<span class="total"></span></span></dd>';
            if (in_array(2, $fieldTemp)) {
                $dd .= '<dd><span class="left">姓　　名：</span><span class="right">' .
                    '<input type="text" name="name" class="text"></span></dd>';
            }
            if (in_array(3, $fieldTemp)) {
                $dd .= '<dd><span class="left">联系电话：</span><span class="right">' .
                    '<input type="text" name="tel" class="text"></span></dd>';
            }
            if (in_array(4, $fieldTemp) && in_array(5, $fieldTemp)) {
                $dd .= '<dd><span class="left">所在地区：</span><span class="right">' .
                ...
                $dd .= "\r\n    ";
                $dd .= '<div class="district1">';
                $dd .= '<input type="hidden" name="province">';

                $dd .= '<input type="hidden" name="city">';
                $dd .= '<input type="hidden" name="county">';
                $dd .= '<input type="hidden" name="town">';
                $dd .= '<dd><span class="left">省　　份：</span><span class="right">' .
                    '<select class="province" lay-filter="province"><option value="">请选择省份</option></select></span>' .
                    '</dd>';
                $dd .= '<dd><span class="left">城　　市：</span><span class="right">' .
                    '<select class="city" lay-filter="city"><option value="">请选择城市</option></select></span></dd>';
                $dd .= '<dd><span class="left">区 / 县 ：</span><span class="right">' .
                ...
                $dd .= '<dd><span class="left">乡镇/街道：</span><span class="right">' .
            ...
            if (in_array(6, $fieldTemp)) {
                $dd .= '<dd><span class="left">详细地址：</span><span class="right">' .
                    '<input type="text" name="address" class="text"></span></dd>';
            }
            if (in_array(7, $fieldTemp)) {
                $dd .= '<dd class="textarea"><span class="left">备　　注：</span><span class="right">' .
                    '<textarea name="note"></textarea></span></dd>';
            }
            if (in_array(8, $fieldTemp)) {
                $dd .= '<dd><span class="left">电子邮箱：</span><span class="right">' .
            ...
            if ($templateOne['is_captcha']) {
                $dd .= '<dd><span class="left">验 证 码：</span><span class="right">' .
                    '<input type="text" name="captcha" class="text2"></span></dd>';
                $dd .= '<dd class="captcha"><span class="left"></span><span class="right"><img src="' .
            ...
            $dd .= '<dd class="pro"><span class="protext">订购商品：</span><span class="prolist">' .
                $productHtml . '</span></dd>';
            $dd .= in_array(1, $fieldTemp) ?
                '<dd>订购数量：<input type="text" name="count" value="1" class="text"></dd>' :
                '<input type="hidden" name="count" value="1">';
            $dd .= '<dd>总价合计：<span class="total"></span></dd>';
            if (in_array(2, $fieldTemp)) {
                $dd .= '<dd>姓　　名：<input type="text" name="name" class="text"></dd>';
            }
            if (in_array(3, $fieldTemp)) {
                $dd .= '<dd>联系电话：<input type="text" name="tel" class="text"></dd>';
            }
            if (in_array(4, $fieldTemp) && in_array(5, $fieldTemp)) {
                $dd .= '<dd>所在地区：<label><input type="radio" name="district_type" value="0" checked>选择填写' .
                ...
                $dd .= '<input type="hidden" name="county">';
                $dd .= '<input type="hidden" name="town">';
                $dd .= '<dd>省　　份：<select class="province" lay-filter="province">' .
                    '<option value="">请选择省份</option></select></dd>';
                $dd .= "\r\n    " .
                    '<dd>城　　市：<select class="city" lay-filter="city"><option value="">请选择城市</option></select></dd>';
                $dd .= '<dd>区 / 县 ：<select class="county" lay-filter="county">' .
                    '<option value="">请选择区/县</option></select></dd>';
                $dd .= '<dd class="town-dd">乡镇/街道：<select class="town" lay-filter="town">' .
                    '<option value="">若不清楚乡镇/街道，可不选</option></select></dd>';
                $dd .= '</div>';
                $dd .= '<div class="district2">';
                $dd .= '<dd>省　　份：<input type="text" name="province2" class="text"></dd>';
                $dd .= '<dd>城　　市：<input type="text" name="city2" class="text"></dd>';
                $dd .= '<dd>区 / 县 ：<input type="text" name="county2" class="text"></dd>';
                $dd .= '<dd>乡镇/街道：' .
                ...
                $dd .= '<dd>区 / 县 ：<select class="county" lay-filter="county">' .
                    '<option value="">请选择区/县</option></select></dd>';
                $dd .= '<dd>乡镇/街道：<select class="town" lay-filter="town">' .
                    '<option value="">若不清楚乡镇/街道，可不选</option></select></dd>';
                $dd .= '</div>';
            } elseif (in_array(5, $fieldTemp)) {
                $dd .= "\r\n    " .
            ...
            if (in_array(6, $fieldTemp)) {
                $dd .= '<dd>详细地址：<input type="text" name="address" class="text"></dd>';
            }
            if (in_array(7, $fieldTemp)) {
                $dd .= '<dd class="textarea"><span>备　　注：</span><textarea name="note"></textarea></dd>';
            }
            if (in_array(8, $fieldTemp)) {
                $dd .= "\r\n    " .
                    '<dd>电子邮箱：<input type="text" name="email" placeholder="选填，可接收发货信息" class="text"></dd>';
            }
            $dd .= ($payHtml ? '<dd>支付方式：' . $payHtml . '</dd>' :
                    '<input type="hidden" name="payment_id" value="' . $templateOne['payment_default'] . '">');
            if ($templateOne['is_captcha']) {
                $dd .= '<dd class="captcha"><span>验 证 码：' .
        ...
        $dd .= '<dd class="submit"><input type="submit" value="立即提交订单"></dd>';

        if (in_array($templateOne['template'], [0, 1, 2, 3])) {
            $searchForm = $templateOne['is_show_search'] ? '<form method="get" action="' . Config::get('url.web1') .
...
<div class="tip"></div>
        ...
        return $templates[$templateOne['template']] ?? '';
    }
}
